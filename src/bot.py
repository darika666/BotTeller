...
def get_sum(message):
    food = get_sum_category(1, message.from_user.id)
    game = get_sum_category(2, message.from_user.id)
    study = get_sum_category(3, message.from_user.id)
    text = f"""еда:{food} развлечения: {game} учеба: {study}"""
...
def webhook():
    bot.remove_webhook()
    s = bot.set_webhook(
        url='https://1f1d21790cbf.ngrok.io' + config.token)
    if s:
